...
tau_deg   = df["angle τ/°"].to_numpy()
alpha_deg = df["angle α/°"].to_numpy()
beta_deg  = df["angle β/°"].to_numpy()

# tau_deg   = np.full(len(df), 90)
# alpha_deg = np.full(len(df), 0)
# beta_deg  = np.full(len(df), 0)

# Calculate Joint Velocities (deg/s)
v_tau   = np.abs(np.gradient(tau_deg, time_s))
v_alpha = np.abs(np.gradient(alpha_deg, time_s))
v_beta  = np.abs(np.gradient(beta_deg, time_s))
...
# Forward kinematics
# ----------------------------
def compute_points(tau_deg, alpha_deg, beta_deg):
...
def update(frame):
    ...
    vel_text.set_text(f'Max Joint Vel: {current_max_v / 6:.1f} RPM\n'
...
plt.title("Diving Beetle Hind Legs: Forward Swimming Motion")
plt.xlabel("X (mm)")
plt.ylabel("Y (mm)")
plt.show()