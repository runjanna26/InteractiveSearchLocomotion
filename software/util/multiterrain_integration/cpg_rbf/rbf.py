...
class RBF:
    ...
    def regenerate_target_traj(self, O0_cpg_t, O1_cpg_t, weight):  
        ...
        Output:
        self.M : single value output of CPG-RBF
        '''

        self.K = np.zeros((self.nc, 1))  # this self.K is not the same size with self.K in imitate learning function
        for i in range(self.nc):
        ...
        return self.M[0]    # convert to single value
    
    def regenerate_target_traj_kernel_masking(self, O0_cpg_t, O1_cpg_t, weight):  
        '''
        Reconstruct kernels with CPG on time step using vectorized operations
    ...
    def regenerate_smooth_gated_traj(self, O0_cpg_t, O1_cpg_t, weight_prev, weight_target, base_alpha=0.01):  
        '''
        Calculates kernels and updates weights ONLY when kernels are inactive.
        '''
        # 1. Calculate current kernel activations
        self.K = np.zeros((self.nc, 1))
        for i in range(self.nc):
            b = np.exp(-(np.power((O0_cpg_t - self.cx[i]), 2) + np.power((O1_cpg_t - self.cy[i]), 2)) / self.variance_gaussian)
            self.K[i] = b
            
        K_flat = self.K.flatten()

        # 2. Kernel-Gated Weight Update
        # (1.0 - K_flat) acts as a mask. Active kernels don't change. Silent kernels update.
        weight_new = weight_prev + base_alpha * (1.0 - K_flat) * (weight_target - weight_prev)

        # 3. Calculate final output trajectory using the newly adjusted weights
        self.M = np.matmul(weight_new, self.K)     
        
        return self.M[0], weight_new
    
    def regenerate_second_order_gated_traj(self, O0_cpg_t, O1_cpg_t, weight_prev, weight_vel_prev, weight_target, omega=6.0, dt=0.01):  
        '''
        Calculates kernels and updates weights using a Second-Order Critically Damped
        Filter gated by kernel activity to eliminate sudden acceleration spikes (jerk).
        
        Inputs:
            weight_prev:     1D array of previous weights
            weight_vel_prev: 1D array of previous weight velocities
            weight_target:   1D array of target weights
            omega:           Natural frequency (higher = faster transition convergence)
            dt:              Time step duration (e.g., 0.01 for 100 Hz)
        '''
        # 1. Calculate current kernel activations
        self.K = np.zeros((self.nc, 1))
        for i in range(self.nc):
        ...
        # 2. Second-Order Critically Damped Physics Logic
        # Calculate raw un-gated acceleration needed to head toward the target smoothly
        raw_acceleration = (omega ** 2) * (weight_target - weight_prev) - (2.0 * omega * weight_vel_prev)
        
        # Gate the acceleration using the kernel mask. 
        # Active kernels experience 0 acceleration (they coast smoothly). 
        # Silent kernels accelerate freely toward the new gait structure.
        gated_acceleration = (1.0 - K_flat) * raw_acceleration

        # 3. Integrate acceleration to get new velocity and new position vectors
        weight_vel_new = weight_vel_prev + gated_acceleration * dt
        weight_new     = weight_prev + weight_vel_new * dt

        # 4. Calculate final output trajectory using the newly adjusted weights matrix
        self.M = np.matmul(weight_new, self.K)     
        
        return self.M[0], weight_new, weight_vel_new
