...
# ======================================================
RENDER = True
ROLLOUTS = 1  # Number of parallel rollouts per iteration
...
TOTAL_JOINTS = 2 * 2 * 3 # 2 sides (L/R) * 2 indexes (F/B) * 3 joints (0,1,2)
...
NUM_PARAMETERS_LEARN = NUM_KERNELS * TOTAL_JOINTS
        ...
        for side in LEG_SIDE:
            for index in LEG_INDEX:
                for joint in JOINT_NAMES:  # joint is 0, 1, 2, or 3
                    ...
                    # target_angle = baseline_angle + network_outpute
                    target_angle = baseline_angle 
...
if __name__ == "__main__":
    ...
    # Initialize lists
    base_parameters = np.full(NUM_PARAMETERS_LEARN, BASE_PARAM_INIT, dtype=np.float64)
    
    # Since we aren't using prior knowledge or symmetry offsets, 
    # we just fill these with zeros to satisfy the function arguments.
    left_offsets = np.zeros(NUM_PARAMETERS_LEARN, dtype=np.float64).tolist()
    right_priors = np.zeros(NUM_PARAMETERS_LEARN, dtype=np.float64).tolist()
    
    print(f"Successfully loaded {len(base_parameters)} base parameters from scratch!")
