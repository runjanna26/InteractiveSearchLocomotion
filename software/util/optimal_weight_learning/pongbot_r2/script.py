...
class StickInsectEnv:
    ...
    def reset(self):
        ...
        for _ in range(int(1.0 / self.model.opt.timestep)):
            for name, ctrl in self.controllers.items():
                ...
                # Hold a target of 0.0 while falling
                ctrl.calculate(0.0, q, dq, self.model.opt.timestep)
                self.data.ctrl[self.actuator_ids[name]] = ctrl.get_torque()
    ...
    def step(self, targets):
        ...
        """
        # ==========================================
        # SINE WAVE DEBUG OVERRIDE
        # ==========================================
        amplitude = 0.1                   # Maximum joint angle in radians
        frequency = 2.0 * np.pi * 1.0     # 1 Hz oscillation (1 full swing per second)
        
        # Calculate the sine wave using MuJoCo's internal simulation time
        sine_target = amplitude * np.sin(frequency * self.data.time)
