...
# ==========================================
# 2. Your Wrapper Class Format (Upgraded with Scaler)
# ==========================================
class HAPTR_Classifier:
    def __init__(self, variant="Base", epochs=50, batch_size=32, lr=1e-3, random_state=42):
        ...
        self.model = None
        self.scaler = None  # 🌟 NEW: Embedded Scaler
        self.n_classes = None
        self.seq_len = None
        self.n_inputs = None
    ...
    def fit(self, X, y):
        ...
        self.n_inputs = X_np.shape[2]
        self.n_classes = len(np.unique(y))
        
        # 🌟 NEW: Fit and Apply StandardScaler
        print("Fitting StandardScaler to training data...")
        self.scaler = StandardScaler()
        # Flatten from 3D (Cycles, Timesteps, Features) to 2D (Cycles * Timesteps, Features)
        ...
        print(f"Training HAPTR ({self.variant}) on {n_samples} scaled gait cycles using {self.device}...")
    ...
    def predict(self, x_raw):
        ...
        # 🌟 NEW: Dynamic Feature Slicing (Protects against stray timestamps)
        expected_features = self.scaler.n_features_in_
        if x_raw.shape[1] > expected_features:
            x_raw = x_raw[:, :expected_features]
            
        # 🌟 NEW: Apply Scaler to Input
        x_scaled = self.scaler.transform(x_raw)
        
        # Convert single scaled sample (160x6) to batch format (1x160x6)
    ...
    def save_model(self, filepath="haptr_model_params.pt"):
        """Saves the PyTorch state dictionary, model parameters, and Scaler."""
        if self.model is None:
        ...
        save_dict = {
            ...
            'scaler': pickle.dumps(self.scaler) # 🌟 NEW: Save Scaler
        }
        torch.save(save_dict, filepath)
        print(f"HAPTR parameters and Scaler saved to {filepath}")
    ...
    def load_model(self, filepath="haptr_model_params.pt"):
        ...
        print(f"\n[SUCCESS] HAPTR Model and Scaler loaded from: '{filepath}'")
        print("-" * 40)
        ...
        print(f"   • Scaler Fitted:  True (Expected Features: {self.scaler.n_features_in_})")
        print("-" * 40 + "\n")
